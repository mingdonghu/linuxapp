...
int main(int argc, const char *argv[]) {
  ...
  UDPSocketInterfaceLinux *client_udp_network = new UDPSocketInterfaceLinux();

  client_udp_network->SetRecvCallback(std::bind(&ClientNode::ReadCall, c_node, std::placeholders::_1, std::placeholders::_2)); 

  bool result = client_udp_network->CreateSocket(UDP_CLIENT, argv[1], argv[2]);
  if (!result) {
